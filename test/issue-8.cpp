...
struct Point2D {
    ...
    Point2D()
        : x(std::numeric_limits<dfloat>::quiet_NaN()),
          y(std::numeric_limits<dfloat>::quiet_NaN())
    {}

    Point2D(dfloat x, dfloat y)
        : x(x)
        , y(y)
    {}
};
...
dfloat distance(Point2D const& p1, Point2D const& p2)
{
...
void testRotation(dfloat angle)
{
    ...
    for (size_t i = 0; i < delaunator.triangles.size(); i += 3)
    {
        ...
        dfloat len_1 = distance(p1, p2);
        dfloat len_2 = distance(p2, p3);
